...
var calendarRemindersList = document.getElementById("calendar-reminders-list");
var months = "";
var days = "";
var monthsArr = [
	"January",
	...
	for (var i = 0; i < 6; i++) {
		...
		for (var j = 0; j < 7; j++) {
			if (i === 0 && j < firstDay) {
				...
				var BreakException = {};
				try {
					if (Store.getReminders()) {
						Store.getReminders().forEach((r) => {
							if (
								parseInt(r.date.substring(8)) == date &&
								parseInt(r.date.substring(5, 7)) == month + 1
							) {
								cell.innerHTML = `<span> ${date} </span> <span class="dot"></span>`;
				...
				cell.onclick = function (event) {
					...
					datePicked.innerHTML = date + " " + monthsArr[month];

					Array.from(calendarRemindersList.children).forEach((child) =>
						calendarRemindersList.removeChild(child)
					); //Clearing events.
					...
					Store.getReminders().forEach((r) => {
						if (
							...
							calendarRemindersList.appendChild(li);
						}
					});
				};
